...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const handleTabletModeChanged = (newIsTablet) => {
      console.log("[My] 收到平板模式变化事件:", newIsTablet, "当前值:", isTablet.value);
      if (isTablet.value !== newIsTablet) {
        isTablet.value = newIsTablet;
        console.log("[My] 平板模式已更新为:", newIsTablet);
      }
    };
    ...
    const myPlaylistScrollLeft = common_vendor.ref(0);
    const canScrollMyPlaylistLeft = common_vendor.ref(false);
    const canScrollMyPlaylistRight = common_vendor.ref(false);
    const recentPlaylistScrollLeft = common_vendor.ref(0);
    const canScrollRecentPlaylistLeft = common_vendor.ref(false);
    const canScrollRecentPlaylistRight = common_vendor.ref(false);
    const onMyPlaylistScroll = (e) => {
      const { scrollLeft, scrollWidth, clientWidth } = e.detail;
      myPlaylistScrollLeft.value = scrollLeft;
      canScrollMyPlaylistLeft.value = scrollLeft > 10;
      canScrollMyPlaylistRight.value = scrollLeft + clientWidth < scrollWidth - 10;
    };
    const onRecentPlaylistScroll = (e) => {
      const { scrollLeft, scrollWidth, clientWidth } = e.detail;
      recentPlaylistScrollLeft.value = scrollLeft;
      canScrollRecentPlaylistLeft.value = scrollLeft > 10;
      canScrollRecentPlaylistRight.value = scrollLeft + clientWidth < scrollWidth - 10;
    };
    const initScrollButtonsVisibility = () => {
      setTimeout(() => {
        if (myPlaylists.value.length > 3) {
          canScrollMyPlaylistRight.value = true;
        }
        if (recentPlaylistHistory.value && recentPlaylistHistory.value.length > 2) {
          canScrollRecentPlaylistRight.value = true;
        }
      }, 500);
    };
    ...
    const viewAllRecentPlayed = () => {
      ...
      const url = "/pages/sharelist/index?mode=recent&fromMyMusic=true";
      if (isTablet.value) {
        common_vendor.index.$emit("my-navigate", { url });
      } else {
    ...
    const viewAllDeleted = () => {
      ...
      const url = "/pages/sharelist/index?mode=deleted&fromMyMusic=true";
      if (isTablet.value) {
        common_vendor.index.$emit("my-navigate", { url });
      } else {
        common_vendor.index.navigateTo({ url });
    ...
    const openRecentPlaylist = (playlist) => {
      ...
      const url = `/pages/sharelist/index?source=${source}&link=${encodeURIComponent(link)}&id=${encodeURIComponent(id)}&picUrl=${encodeURIComponent(playlist.coverUrl || "")}&name=${encodeURIComponent(playlist.name || "")}&fromName=my`;
      if (isTablet.value) {
        common_vendor.index.$emit("my-navigate", { url });
      } else {
        common_vendor.index.navigateTo({ url });
    ...
    const openPlaylist = (playlist) => {
      if (!playlist)
        return;
      if (playlist.source === "default") {
        const url = `/pages/sharelist/index?mode=default&fromMyMusic=true`;
        if (isTablet.value) {
          common_vendor.index.$emit("my-navigate", { url });
        } else {
          common_vendor.index.navigateTo({ url });
      ...
      if (playlist.source === "love") {
        const url = `/pages/sharelist/index?mode=love&fromMyMusic=true`;
        if (isTablet.value) {
          common_vendor.index.$emit("my-navigate", { url });
        } else {
          common_vendor.index.navigateTo({ url });
      ...
      if (playlist.source === "user") {
        const url = `/pages/sharelist/index?mode=user&fromMyMusic=true&playlistId=${encodeURIComponent(playlist.id)}&playlistName=${encodeURIComponent(playlist.name)}`;
        if (isTablet.value) {
          common_vendor.index.$emit("my-navigate", { url });
        } else {
          common_vendor.index.navigateTo({ url });
      ...
      if (playlist.source === "imported") {
        ...
        const url = `/pages/sharelist/index?source=${platform}&link=${encodeURIComponent(playlistId)}&platform=${encodeURIComponent(platformNames[platform] || platform)}&preview=true&fromMyMusic=true&playlistId=${encodeURIComponent(playlist.id)}&coverUrl=${encodeURIComponent(playlist.coverUrl)}&playlistName=${encodeURIComponent(playlist.name)}`;
        if (isTablet.value) {
          common_vendor.index.$emit("my-navigate", { url });
        } else {
          common_vendor.index.navigateTo({ url });
    ...
    common_vendor.onMounted(() => {
      ...
      console.log("[My] 已注册监听: themeChanged, tabletModeChanged");
      checkIsTablet();
    ...
    common_vendor.onUnmounted(() => {
      ...
      console.log("[My] 已移除监听: themeChanged, tabletModeChanged");
    });
    return (_ctx, _cache) => {
